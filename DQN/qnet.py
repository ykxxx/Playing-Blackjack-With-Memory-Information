...
class Qnet:
    def __init__(self, config, sess, target=False):
        ...
        self.scope_name = "target" if target else "primary"
        ...
        with tf.variable_scope(self.scope_name):
            ...
            self.input = tf.concat([self.obv, self.memory], axis=-1)
            ...
            for i in range(config.num_layer - 1):
                ...
                self.layers.append(tf.layers.dense(self.layers[-1], num_nodes, activation=activation, kernel_initializer=initializer))
            ...
            if self.config.decay_lr:
                self.learning_rate = tf.train.exponential_decay(self.config.learning_rate, self.global_step, self.decay_step, self.decay_rate, staircase=True)
            else:
    ...
    def get_q_values(self, obv, memory):
    ...
    def fit_target_q(self, obv, memory, action_chosen, target_q, q_mask):

        optimizer, loss = self.sess.run([self.optimizer, self.loss], feed_dict={
            self.obv: obv,
            self.memory: memory,
            # self.hidden_states: hidden_states,
